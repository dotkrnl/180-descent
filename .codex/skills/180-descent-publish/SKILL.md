...
4. After the user says the refinement pass is done, check `rtk git status -sb` and inspect the source diff. Confirm refinements are in tracked source files under `src/` or skill files, never only `_site/`.
