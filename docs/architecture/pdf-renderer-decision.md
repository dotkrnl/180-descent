...
- `src/content/days/###-slug/` is the canonical route path; `day.yaml` remains the canonical manifest for day number, block, localized metadata, appendices, and interaction scripts.
...
- successful XeTeX logs contain no overfull or underfull boxes, missing glyphs,
  or LaTeX/package/font warnings.
